...
def fs_read_attr_comid(dir_db_attrs:str | os.PathLike, comids_resp:list | Iterable, attrs_sel: str | Iterable = 'all',
    ...
    :param _s3: _description_, defaults to None
    :type _s3: _type_, optional
    :param storage_options: _description_, defaults to None
    :type storage_options: _type_, optional
...
def _check_attributes_exist(df_attr: pd.DataFrame, attrs_sel:pd.Series | Iterable) -> Dict[pd.DataFrame, pd.Series]:
    """ Checks if any COMIDs have different numbers of attributes 
        It's expected that they all have the same attributes.
...
def _find_feat_srce_id(dat_resp: Optional[xr.core.dataset.Dataset] = None,
    ...
    """ Try grabbing :mod:`fsds_proc` standardized dataset attributes (preferred in cases of 
        processing multiple datasets & attributes differ by dataset).
        Otherwise, fallback on config file. At least one argument
        must be provided.
...
def fs_retr_nhdp_comids(featureSource:str,featureID:str,gage_ids: Iterable[str] ) ->list:    
    ...
    :param featureSource: the datasource for featureID from the 
      R function :mod:`nhdplusTools` :func:`get_nldi_features()`, e.g. 'nwissite'
    :type featureSource: str
    :param featureID: The conversion format of `gage_ids` into a recognizable string
      for :mod:`nhdplusTools`, which is an f-string configured conversion of `gage_id`
      e.g. `'USGS-{gage_id}'`. Expected to contain the string `"{gage_id}"`
    :type featureID: str
...
class AlgoTrainEval:
    def __init__(self, df: pd.DataFrame, attrs: Iterable[str], algo_config: dict,
                 ...
                 verbose: bool = False):
        """ The aglorithm training and evaluation class
        ...
        :param algo_config: The algorithm configuration as read from the algo_config yaml
          where each key is the algorithm that will be run. Presently allowable keys include:
            ...
            Each algorithm key contains sub-dict keys for the parameters that may be passed
            to the corresponding :mod:`sklearn` algorithm. If no parameters keys are passed, the 
            :mod:`sklearn` algorithm's default arguments are used.
        ...
        :param test_size: parameter for :func:`sklearn.model_selection.train_test_split`, 
            represents proportion of dataset to include in the test split. defaults to 0.3
    ...
    def split_data(self):
        """Split dataframe into training and testing predictors (X) and response (y) variables
            using :func:`sklearn.model_selection.train_test_split`
        """
    ...
    def evaluate_algos(self) -> dict:
        ...
        :return: Evaluation of algorithm performance from the test dataset
         The dict keys include the following:
    ...
    def org_metadata_alg(self):
        """Must be called after running AlgoTrainEval.save_algos()
            Records saved location of trained algorithm
        """
    ...
    def train_eval(self):
        """ The overarching train, test, evaluation wrapper that also saves 
        algorithms and evaluation results
        """
